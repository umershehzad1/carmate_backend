...
/**
 * CarPages Scraper
 * Scrapes vehicle listings from CarPages.ca
 */
class CarPagesScraper extends BaseScraper {
  constructor() {
    super("CarPages");
    this.baseUrl = "https://www.carpages.ca";
    this.searchUrl = `${this.baseUrl}/used-cars/`;
  ...
  async scrape() {
    this.initStats();
    this.log("Starting CarPages scraper...");
  ...
  async scrapePage(page) {
    ...
    const vehicles = [];
    const listings = $(".vehicle-card, .listing-card, [class*='vehicle-item']");
  ...
  async extractVehicleData($, listingElement) {
    try {
      ...
      const titleElement = listingElement.find("h2, h3, .vehicle-title, [class*='title']").first();
      ...
      const location = listingElement
        .find(".location, [class*='location'], .dealer-info")
        .first()
        .text()
        .trim();
      
      const image = listingElement
        .find("img")
        .first()
        .attr("src") || listingElement.find("img").attr("data-src");
      ...
      const details = {};
      listingElement.find("li, .spec-item, [class*='spec']").each((i, elem) => {
        const text = $(elem).text().trim();
        this.parseDetailText(text, details);
      });

      // Get dealer info
      const dealerName = listingElement
        .find(".dealer-name, [class*='dealer']")
        .first()
        .text()
        .trim() || "CarPages Dealer";
      ...
      const vehicleData = {
        ...
        // Source
        sourceUrl: detailUrl ? (detailUrl.startsWith("http") ? detailUrl : `${this.baseUrl}${detailUrl}`) : null,
  ...
  async scrapeDetailPage(detailPath) {
    ...
    $(".gallery img, .photo-gallery img, [class*='vehicle-image'] img, [class*='photo'] img, picture img").each((i, elem) => {
    ...
    const description = $(".description, [class*='description'], .vehicle-description, .comments")
      .first()
      .text()
      .trim();
    if (description) {
    ...
    $(".specs li, .specifications li, [class*='spec-item'], .features li, dl dt, dl dd, .vehicle-info li").each((i, elem) => {
      const text = $(elem).text().trim();
      const label = $(elem).find("strong, .label, dt").text().trim().toLowerCase();
      const value = text.replace(label, "").replace(":", "").trim() || $(elem).find(".value, dd").text().trim();
    ...
    const dealerPhone = $(".dealer-phone, [class*='phone'], a[href^='tel:']").first().text().trim();
    if (dealerPhone) {
  ...
  parseDetailText(text, details) {
    ...
    if (lowerText.includes("km") || lowerText.includes("mileage")) {
      ...
      details.transmission = lowerText.includes("automatic")
        ? "Automatic"
        : "Manual";
    } else if (
      ...
      if (lowerText.includes("gas") || lowerText.includes("gasoline")) details.fuelType = "Gas";
      if (lowerText.includes("diesel")) details.fuelType = "Diesel";
      if (lowerText.includes("electric")) details.fuelType = "Electric";
      if (lowerText.includes("hybrid")) details.fuelType = "Hybrid";
    } else if ((lowerText.includes("l") || lowerText.includes("cc") || lowerText.includes("cylinder") || lowerText.includes("engine")) && (lowerText.match(/\d+\.\d+l/i) || lowerText.match(/\d+\s*cc/i))) {
  ...
  mapSpecification(label, value, specs) {
    ...
    if (lowerLabel.includes("mileage") || lowerLabel.includes("km") || lowerLabel.includes("odometer")) {
      specs.mileage = value;
      ...
      specs.doors = this.extractNumber(value);
...
module.exports = CarPagesScraper;
