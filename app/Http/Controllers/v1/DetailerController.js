...
o.getAllDetailers = async function (req, res, next) {
  try {
    const { page = 1, limit = 10, search } = req.query;
    const offset = (page - 1) * limit;
    const where = {};

    // Search detailers by city (case-insensitive, partial match)
    if (search) {
...
o.getDetailerDetails = async function (req, res, next) {
  try {
    const { slug } = req.params;
    const repair = await Detailer.findOne({
      where: { slug },
      include: [
    ...
    if (!repair) {
      return json.errorResponse(res, "Detailer not found", 404);
    }
...
o.updateDetailerProfile = async function (req, res, next) {
  try {
    ...
    const updatedDetailer = await repair.update(updateData);

    // Step 5: Return response with the updated record
    return json.successResponse(
      res,
      "Detailer profile updated successfully",
      200
    );
  } catch (error) {
    console.error("updateDetailerProfile Error:", error);
    return json.errorResponse(res, error.message || error, 400);
  }
};

o.deleteDetailerProfile = async function (req, res, next) {
  try {
    // Ensure only Detailer can perform this action
...
o.getDetailerStats = async function (req, res, next) {
  try {
    ...
    console.error("Error in getDetailerStats:", error);
    return json.errorResponse(res, error.message || error, 400);
  }
};
